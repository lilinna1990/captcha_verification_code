...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=1, augment=False, hyp=None, rect=True, image_weights=False,
                 cache_images=False):
        path = str(Path(path))  # os-agnostic
        with open(path, 'r') as f:
            ...
            for i in range(len(paths)):
                self.img_files.append(paths[i].split(' ')[0])
                #print(self.img_files[i])
            
            #self.img_files = [ x.split(' ')[0] for x in f.read().splitlines()] # os-agnostic
        ...
        # Define labels
        self.label_files = [x.replace('dataset_mongtage_small', 'cocoxml').replace(os.path.splitext(x)[-1], '.txt')
